...
const sendEmailTool = tool({
  ...
  execute: async ({ to, subject, html }) => {
    ...
    return `Email sent to ${to} with subject ${subject} and body ${html}`;
  },
});
    ...
    for (const interput of result.interruptions) {
      if (interput.type === 'tool_approval_item') {
        const isAllowed = await askForUserConfitmation(
          `Agent ${interput.agent.name} is asking for calling tool ${interput.name} with args ${JSON.stringify(interput.arguments)}. Do you want to allow?`
        );
        if (isAllowed) {
...
main('What is the weather of Karachi, Lahore and Islamabad and send me on kamran186@gmail.com');